  ...
  protected function createUser($save = false)
  {
    ...
    foreach($config as $field => $field_config)
    {
      ...
      if(!is_null($call))
      {
		...
		{
		  //sfContext::getInstance()->getLogger()->info('createUsercreateUsercreateUser'.$result.'bbb'.$path);
		}
	...
	if (sfConfig::get('sf_logging_enabled'))
	{
	  sfContext::getInstance()->getLogger()->info('show this only in first time user connect to fb');
	}
    }
